...
const DashboardProfileSidebar = ({}: Props) => {
  ...
  const [url, setUrl] = useState<string>("");
  const [searchParams, setSearchParams] = useSearchParams();
  const current = searchParams.get("current") || "entity";
  ...
  useEffect(() => {
    const currentItem = profileSideBarList.find((item) => item.url === current);
    if (currentItem) {
  ...
  const lastName = sessionStorage.getItem("lastName") || "Last";
