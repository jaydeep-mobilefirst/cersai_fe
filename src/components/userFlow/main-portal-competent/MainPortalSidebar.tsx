  ...
  useEffect(() => {
    ...
    if (sessionData) {
      ...
      const dashboardRoles = rolesArray.filter(
        (role) => role === "dashboard-viewer-role-competent-authority"
      );
      ...
      const schemeRolesView = rolesArray.filter(
        (role) => role === "scheme-view-access-competent-authority"
      );
      ...
      const userRoles = rolesArray.filter(
        (role) => role === "role-creation-access-competent-authority"
      );
      if (userRoles?.length > 0) {
        setUser(true);
      }
      const roleRoles = rolesArray.filter(
        (role) => role === "user-creation-access-competent-authority"
      );
