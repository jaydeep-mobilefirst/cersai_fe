      ...
      >
        ...
        ?
          allSelectedOptions?.map((s) => {
            return <span className="bg-blue-100 py-1 px-1">{s.label}<span className="text-red-500 ml-1 hover:bg-red-100 p-1" onClick={() => remove(s)}>X</span></span>
          })
          :
          "Select"
        }
          ...
          >
            ...
            {multiselect && options.map((option, index) => (
              <a
                ...
                onClick={() => setOption(option)}
                href="#"
                className={`px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-gray-900 ${allSelectedOptions?.find((f) => f.value === option.value) && 'bg-blue-50'}`}
                role="menuitem"
              >
                {option.label}
              </a>
