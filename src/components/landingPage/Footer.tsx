          ...
          <>
            {homePageData?.homePageData?.footer?.map((link:any,index:any) => {
              if (link.heading === "Sitemap") {
                return (
                  <p className="block px-2 py-1 border-white md:inline-block pr-6 text-gilroy-medium"  key={index}>
                    {link.text.toUpperCase()}
                  </p>
                );
              }
              return (
                <p className="block px-2 py-1 md:border-r border-white md:inline-block pr-6 text-gilroy-medium" key={index}>
                  {link.text.toUpperCase()}
                </p>
              );
            })}
