...
interface ProfileRegulatorState {
  ...
  nodaldetailsStore: () => Promise<void>; // Async function to update entity details
}

const useProfileNodalStore = create<ProfileRegulatorState>((set, get) => ({
  formData: [], // Initial state for formData
  setFormData: (data: FormDataItem[]) => set({ formData: data }), // Function to update formData

  nodaldetailsStore: async () => {
...
export default useProfileNodalStore;
