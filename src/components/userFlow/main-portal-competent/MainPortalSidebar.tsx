...
const MainPortalSidebar = ({ layout }: Props) => {
  ...
  useEffect(()=>{
    ...
    if (sessionData) {
      ...
      const dashboardRoles = rolesArray.filter(role =>
        role === "dashboard-viewer-role-competent-authority"
      );
      ...
      const schemeRolesView = rolesArray.filter(role =>
        role === "scheme-view-access-competent-authority"
      );
      if(schemeRolesView?.length > 0){
      setSchemeView(true);
      }
      const schemeRoles = rolesArray.filter(role =>
        role === "scheme-edit-access-competent-authority"
      );
      ...
      setScheme(true);
       ...
       const userRoles = rolesArray.filter(role =>
        role === "role-creation-access-competent-authority"
      );
      if(userRoles?.length > 0){
      setUser(true);
      }
      const roleRoles = rolesArray.filter(role =>
        role === "user-creation-access-competent-authority"
      );
  ...
  useEffect(() => {
    if (!isActive || refreshPage == '5') {
      ...
      Swal.fire({
        icon: "error",
        title: refreshPage == '5' ? "Dont refresh the page. Please login again" : "User inactive for 10 min. Please login again",
      });
      navigate("/");
    }
  });
